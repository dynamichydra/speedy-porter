...
if(isset($_SESSION['user_type']) && $_SESSION['user_type'] != 'staff'){
 ?>
                <div class="pull-right">
                    <a class="btn  btn-primary" href="<?php echo base_url('admin/add_data/create_station'); ?>">Add new Zip code</a>
                </div>
                <?php
              }
              ?>
                           ...
                           ?>
                                ...
                                <tbody>
                                    ...
                                    foreach ($row as $k => $v) {
                                      // $i++
                                        ?>
                                        <tr data-id="<?php echo $v['stationID'];?>">
                                          <td><?php echo $i++ ?></td>
                                            <td><?php echo $v['district_name'] ?></td>
                                            <td><?php echo $v['station_name'] ?></td>
                                            <?php if($v['area'] == 'urban'){$area = "Urban";}else{$area = $v['area'];}?>
                                            <td><?php echo $area ?></td>
                                            <?php
                              if(isset($_SESSION['user_type']) && $_SESSION['user_type'] != 'staff'){
                               ?>
                                            <td><button class="btn btn-default btn-icon-anim btn-square list-button" onclick="actionFunction()" data-tag="edit" data-url="<?php echo base_url("admin/add_data/create_station/".$v['stationID']);?>"><i class="fa fa-pencil"></i></button>&nbsp;<button class="btn btn-info btn-icon-anim btn-square list-button" data-title="Zip code" data-tag="delete" onclick="actionFunction()" data-url="<?php echo base_url("admin/add_data/delete_station/".$v['stationID']);?>"><i class="fa fa-trash-o"></i></button></td>
                                            <?php
                                          }
                                          ?>
                                        </tr>
