...
class Api extends CI_Controller {
    ...
    public function login(){
      ...
      if(is_numeric($email)){
        $res=$this->api_model->get_where_all('customer',['phone'=>  $email,'password'=> $pass ]);
      }else{
      $res=$this->api_model->get_where_all('customer',['email'=>  $email,'password'=> $pass ]);
    }
      // print_r($res[0]);die;
      if(count($res)>0){
            ...
            if ($isValidEmail && $isValidPhone) {
                ...
                $sub="SPEEDY PORTER Registration Successfull";    //subject
         			 $msg= /*-----------email body starts-----------*/
         				 'Dear Customer, '.$txtName.' thank you for registering with us,please use below credentials to login to your account,
   ...
   //   // $arr['lmt'] = 10;
   //   $data = array();
   //   $data['row'] = $this->api_model->get_all_consignment_aadmin($arr);
   ...
   public function consedit(){
     ...
     $data = array();
     $data['row'] = $this->api_model->get_where_all('order_master', ['id' => $consid]);
   ...
   public function trackdetail(){
     $this->output->set_content_type('application/json');
      $consid = $this->input->post('considtofetch');
       $getid= $this->api_model->get_where_all('order_master', [' id' => $consid]);
      $consignmentId = $getid[0]['consignment_id'];

     $response['consignment_id'] = $this->api_model->get_where_all('order_master', ['consignment_id' => $consignmentId]);
   ...
   public function volumetric(){
     ...
     $data = array();
     $data['vol'] = $this->api_model->get_allheight();
   ...
   public function volumetric_width(){
     ...
     $data = array();
     $data['vol'] = $this->api_model->get_where_all('volumetric',['height'=>$height,'status'=>'active']);
   ...
   public function submitorder(){
     $this->output->set_content_type('application/json');
       ...
       $parcel_wt = $this->input->post('parcel_wt');
       ...
       $payment_at = $this->input->post('payment_at');
       $delivery_charge = intval($this->input->post('delivery_charge'));

         if($delivery_charge != ""){
         $consid = $this->api_model->insert('order_master', [
  ...
  public function getconsid(){
    $this->output->set_content_type('application/json');
    $response['autono'] = 'SP'.$this->random_strings(8);
    $response['message'] = "success";
    $response['success'] = true;
    $this->output->set_output(json_encode($response));
  }
...
function getpackagepricing() {
        $this->output->set_content_type('application jason');
        $package = $this->api_model->get_where_all('package',['status' => 'active']);
